...
export default function HostPage(){
  ...
  async function submit(e:FormEvent){
    ...
    try{const saved=await saveListingRemote(next, form.ownerEmail || undefined);setListing(saved)}catch(error){setError(error instanceof Error?error.message:"Could not save this analysis.")}
    finally{setSaving(false)}
  }

  return <main className="wrap section topspace"><div className="grid grid2"><div><p className="kicker">Calendar advisor</p><h1 className="big">Paste your Airbnb or VRBO listing URL.</h1><p className="muted">GapStay reads your calendar feed when available, finds open gaps, scores the next window, and shows whether to wait or package it as a furnished stay.</p>
...
function Result({listing}:{listing:Listing}){const a=listing.analysis;return <div><p className="kicker">GapStay recommendation</p><h2 className="big">{a.recommendation==="furnished"?"Package this gap":"Hold for short stays"}</h2><p className="muted">{prettyDate(a.startDate)} - {prettyDate(a.endDate)} · {a.nights} nights · furnished-stay offer {money(a.recommendedTotal)}</p><div className="metric" style={{marginTop:18}}><b>Best next move</b><p className="small">{a.recommendation==="furnished"?"Start marketing the furnished-stay offer while keeping Airbnb or VRBO live until a renter is ready.":"Keep the nightly listing active and re-check if the gap remains open closer to arrival."}</p></div><div className="result" style={{marginTop:18}}><div className="stat"><span>Best-case nightly</span><strong>{money(a.fullNightRevenue)}</strong></div><div className="stat"><span>Likely short-stay</span><strong>{money(a.likelyShortTermRevenue)}</strong></div><div className="stat"><span>Furnished offer</span><strong>{money(a.recommendedTotal)}</strong></div><div className="stat"><span>Fill probability</span><strong>{Math.round(a.fillProbability*100)}%</strong></div></div><ul className="list">{a.explanation.map(x=><li key={x}>{x}</li>)}</ul><div className="actions"><CheckoutButton plan="publish_gap" listingId={listing.id}>Publish this offer — $19</CheckoutButton><Link className="pill" href={`/listing/${listing.id}`}>Preview offer</Link></div></div>}
