...
export default function HostPage(){
  ...
  return <main className="wrap section topspace"><div className="grid grid2">
    ...
    <div className="card pad" style={{alignSelf:"start",position:"sticky",top:96}}>{listing?<Result listing={listing}/>:<p className="muted">Your result will appear here. It will include likely short-term revenue, recommended furnished-stay price, monthly equivalent, timing status, and a public listing link.</p>}</div>
  </div></main>
}
...
function Result({listing}:{listing:Listing}){const a=listing.analysis;const t=timingCall(a.startDate,a.nights);return <div><p className="kicker">Recommended offer</p><h2 className="big">{money(a.recommendedTotal)}</h2><p className="muted">{prettyDate(a.startDate)} - {prettyDate(a.endDate)} · {a.nights} nights · {money(a.monthlyEquivalent)}/mo equivalent</p><div className="metric" style={{marginTop:18}}><b>{t.label}</b><p className="small">{t.text}</p></div><div className="result" style={{marginTop:18}}><div className="stat"><span>Full nightly target</span><strong>{money(a.fullNightRevenue)}</strong></div><div className="stat"><span>Likely short-term</span><strong>{money(a.likelyShortTermRevenue)}</strong></div><div className="stat"><span>Saved turns</span><strong>{money(a.turnoverSavings)}</strong></div><div className="stat"><span>Certainty</span><strong>{a.certaintyScore}%</strong></div></div><ul className="list">{a.explanation.map(x=><li key={x}>{x}</li>)}</ul><div className="actions"><Link className="darkpill" href={`/listing/${listing.id}`}>Open listing</Link><Link className="pill" href="/dashboard">Dashboard</Link></div></div>}
