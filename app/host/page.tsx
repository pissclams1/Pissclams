...
export default function HostPage(){
  ...
  async function submit(e:FormEvent){
    e.preventDefault();
    setSaving(true);
    const analysis=analyzeGap({startDate:form.startDate,endDate:form.endDate,nightlyTarget:Number(form.nightlyTarget),expectedOccupancy:Number(form.expectedOccupancy),cleaningFee:Number(form.cleaningFee),market:form.market as "soft"|"balanced"|"hot"});
    const next:Listing={id:`listing-${Date.now().toString(36)}`,title:form.title,city:form.city,state:form.state,propertyType:form.propertyType,bedrooms:Number(form.bedrooms),bathrooms:Number(form.bathrooms),hostName:form.hostName,description:form.description,amenities:["Fast Wi-Fi","Workspace","Kitchen","Laundry","Parking","Utilities included"],rules:["No smoking","Normal checkout cleaning","Quiet hours after 10 PM"],analysis,sourceUrl:form.sourceUrl||undefined,imageUrl:imported?.image,rating:imported?.signals?.rating,reviews:imported?.signals?.reviews,guestCount:imported?.signals?.guests,nightlyRate:imported?.signals?.nightly};
