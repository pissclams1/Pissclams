...
export default function HostPage(){
  ...
  return <main className="wrap section topspace"><div className="grid grid2">
    <div><p className="kicker">Free gap scan</p><h1 className="big">Check one open stretch before you subscribe.</h1><p className="muted">Enter an open calendar gap, your current nightly target, and expected occupancy. GapStay compares the nightly path against a furnished-stay offer and shows the kind of recommendation Calendar Watch sends automatically.</p>
...
function timingCall(startDate:string,nights:number){const days=Math.round((new Date(`${startDate}T12:00:00`).getTime()-Date.now())/86400000);if(days>90)return{label:"Watch",text:"Too early to act. Keep tracking it, but this gap is long enough to keep on the radar."};if(days>60)return{label:"Package now",text:"This is the backup-offer window. Keep Airbnb live, but build the furnished-stay page before monthly guests move on."};if(days>45)return{label:"Act now",text:"Start sharing the page. Monthly guests are still possible, but the window is closing."};if(days>30)return{label:"Late",text:"You can still try, but waiting has already reduced the monthly-stay pool."};return{label:"Salvage",text:"Under 30 days is usually discounted nightly, local outreach, or partial-stay salvage."}}
function Result({listing}:{listing:Listing}){const a=listing.analysis;const t=timingCall(a.startDate,a.nights);return <div><p className="kicker">GapStay recommendation</p><h2 className="big">{t.label}</h2><p className="muted">{prettyDate(a.startDate)} - {prettyDate(a.endDate)} · {a.nights} nights · suggested furnished-stay offer {money(a.recommendedTotal)}</p><div className="metric" style={{marginTop:18}}><b>Best next move</b><p className="small">{t.text}</p></div><div className="metric" style={{marginTop:14}}><b>Annual revenue beats nightly rate.</b><p className="small">The goal is not the highest nightly rate on a screenshot. The goal is the most money collected across the year after empty nights, platform fees, cleaning turns, and carrying costs.</p></div><div className="result" style={{marginTop:18}}><div className="stat"><span>Full nightly target</span><strong>{money(a.fullNightRevenue)}</strong></div><div className="stat"><span>Likely short-term</span><strong>{money(a.likelyShortTermRevenue)}</strong></div><div className="stat"><span>GapStay offer</span><strong>{money(a.recommendedTotal)}</strong></div><div className="stat"><span>Saved turns</span><strong>{money(a.turnoverSavings)}</strong></div></div><ul className="list">{a.explanation.map(x=><li key={x}>{x}</li>)}<li>Calendar Watch keeps checking instead of making you remember to look.</li><li>When the signal changes, GapStay sends the alert and gives you the page to post.</li></ul><div className="actions"><Link className="darkpill" href="/pricing">Turn on Calendar Watch</Link><Link className="pill" href={`/listing/${listing.id}`}>Open sample page</Link></div></div>}
