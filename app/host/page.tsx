...
import { saveListing, type Listing } from "../../lib/store";

export default function HostPage(){
  const [form,setForm]=useState({startDate:"2026-07-03",endDate:"2026-08-14",nightlyTarget:"210",expectedOccupancy:"43",cleaningFee:"195",market:"balanced",title:"Quiet furnished stay with workspace",city:"Marco Island",state:"FL",propertyType:"Condo",bedrooms:"2",bathrooms:"2",hostName:"Host",description:"A furnished mid-term stay with fast Wi-Fi, kitchen, laundry, parking, and transparent all-in pricing."});
  const [listing,setListing]=useState<Listing|null>(null);
  function update(key:string,value:string){setForm(prev=>({...prev,[key]:value}))}
  function submit(e:FormEvent){e.preventDefault(); const analysis=analyzeGap({startDate:form.startDate,endDate:form.endDate,nightlyTarget:Number(form.nightlyTarget),expectedOccupancy:Number(form.expectedOccupancy),cleaningFee:Number(form.cleaningFee),market:form.market as any}); const next:Listing={id:`listing-${Date.now().toString(36)}`,title:form.title,city:form.city,state:form.state,propertyType:form.propertyType,bedrooms:Number(form.bedrooms),bathrooms:Number(form.bathrooms),hostName:form.hostName,description:form.description,amenities:["Fast Wi-Fi","Workspace","Kitchen","Laundry","Parking","Utilities included"],rules:["No smoking","Normal checkout cleaning","Quiet hours after 10 PM"],analysis}; saveListing(next); setListing(next)}
  return <main className="wrap section topspace"><div className="grid grid2">
    <div><p className="kicker">Host analyzer</p><h1 className="big">Turn vacant dates into a realistic mid-term offer.</h1><p className="muted">Enter a calendar gap, your current nightly target, and expected occupancy. GapStay calculates the all-in furnished-stay offer.</p>
      <form onSubmit={submit} className="card pad grid" style={{marginTop:24}}>
        ...
        <button className="darkpill" type="submit">Generate GapStay offer</button>
      </form>
    </div>
    <div className="card pad" style={{alignSelf:"start",position:"sticky",top:96}}>{listing?<Result listing={listing}/>:<p className="muted">Your result will appear here. It will include likely short-term revenue, recommended mid-term price, monthly equivalent, and a public listing link.</p>}</div>
  </div></main>
}
