...
export default function DashboardPage(){
  ...
  return <main className="wrap section topspace"><p className="kicker">Host dashboard</p><h1 className="big">Your GapStay dashboard</h1><p className="muted">Everything you have analyzed, published, and heard back on — in one place.</p>
    <section className="section" style={{paddingTop:28}}><div className="heroCardTop"><h2>Gap pages</h2><Link className="darkpill" href="/host">Analyze a gap</Link></div>{listings.length===0?<div className="card pad" style={{marginTop:16}}><h3>No gap pages yet.</h3><p className="muted">Enter your vacant dates and we will show you what the month is worth.</p><Link className="darkpill" href="/host">Analyze a gap</Link></div>:<div className="grid" style={{marginTop:16}}>{listings.map(l=><div className="card pad" key={l.id}><div className="heroCardTop"><div><h3>{l.title}</h3><p className="muted">{l.city}, {l.state} · {prettyDate(l.analysis.startDate)} - {prettyDate(l.analysis.endDate)}</p></div><div style={{textAlign:"right"}}><b style={{fontSize:28}}>{money(l.analysis.recommendedTotal)}</b><p className="small">{money(l.analysis.monthlyEquivalent)}/mo equivalent</p></div></div><div className="actions"><Link className="pill" href={`/listing/${l.id}`}>Open page</Link></div></div>)}</div>}</section>
    <section><h2>Requests</h2>{inquiries.length===0?<div className="card pad"><p className="muted">No requests yet. Publish a gap page and share it where furnished-stay renters are likely to see it.</p></div>:<table className="table card"><thead><tr><th>Name</th><th>Email</th><th>Type</th><th>Message</th></tr></thead><tbody>{inquiries.map(i=><tr key={i.id}><td>{i.name}</td><td>{i.email}</td><td>{i.reason}</td><td>{i.message}</td></tr>)}</tbody></table>}</section>
  </main>
}
